    ...
    <div className="space-y-4">
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between gap-y-2 lg:gap-x-0 gap-x-4">
              <p className="text-base md:text-xl font-body text-[var(--btn-bg-black)] pb-2 lg:w-[95%] w-full">{item.title}</p>
              <button
                onClick={() =>
            ...
            {isOpen && (
              <div className="font-body text-sm md:text-base text-[var(--text-secondary)] w-[95%]">
                {item.content}
              </div>
            )}
          </div>
